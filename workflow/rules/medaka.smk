...
rule medakaX:
  conda: "../envs/medaka.yaml"
	...
	shell:
		"""
		DIR_temp=".medakaX_{wildcards.sample}"
		mkdir -p $DIR_temp
		...
		do
			...
			medaka_consensus -t {threads} -m {params.model} -d $DIR_temp/prev.fa -i {input.fq} -o $DIR_temp >{log} 2>&1
			mv $DIR_temp/consensus.fasta $DIR_temp/prev.fa
		done
